...
Page({
    ...
    scaleFn(e) {
        if (e.touches[0].pageY - this.data.globalY < -50) {
            ...
            if (height > 750 && height !== 800) {
                animationA.width(48).height(48).left('63.5%').bottom('90px').step({
                    duration: 1200
                })
            } else if (height === 800) {
                animationA.width(48).height(48).left('63.5%').top('auto').bottom(58).step({
                    duration: 1200
                })
            } else {
                animationA.width(48).height(48).left('63.5%').top('auto').bottom(58).step({
            ...
            this.setData({
                ...
                myClassStyle_text: "width:40px;height:10px;margin:0px;"
            })

            // 消失动画
            animationA.translateY(58).opacity(1).step({
                delay: 1000,
                duration: 200
            })
            animationA.scale(0).opacity(0).step({
                duration: 800
            })
            setTimeout(() => {
                if (typeof this.getTabBar === 'function' && this.getTabBar()) {
