Page({
    ...
    scaleFn(e) {
        if (e.touches[0].pageY - this.data.globalY < -50) {
            ...
            // 缩小动画
            animationA.scale(0.8).translateX(20).translateY(-60).step({
                duration: 800
            })
            animationC.width("80vw").height("55vh").opacity(1).top("180px").left("10%").step({
                duration: 800
            })

            if (this.data.explorePos !== 0) {
                animationB.translateY(-100).opacity(0).step({
                    duration: 100
                })
            }
            this.setData({
            ...
            animationB.translateY(this.data.explorePos).opacity(1).step({
                duration: 600
            })
            // 展示navbar
            this.setData({
